...
public class Vaccines {
    ...
    private Map<String, List<String>> intervals = new LinkedHashMap<>();
    private List<Integer> week = new ArrayList<>();
    private Map<String, Map<String, Integer>> allocated = new HashMap<>();
    private int num = 0, lc=0, jx=0;
    ...
    public long loadPeople(Reader people) throws IOException, VaccineException {
        ...
        if(!str1.equals("SSN,LAST,FIRST,YEAR")&&lx!=null) lx.accept(lc=1,str1);
        if(row1.length!=4 || !row1[0].equals("SSN") || !row1[1].equals("LAST") ||!row1[2].equals("FIRST") || !row1[3].equals("YEAR"))
            throw new VaccineException();
        while (br.ready()) {
            String str = br.readLine();
            if (str == null) break;
            String[] row = str.split(",");
            if((row.length!=4 || this.people.get(row[0])!=null) && lx!=null) lx.accept(i+(++lc)+1, str);
            if(row.length != 4 || this.people.get(row[0]) != null)
                continue;
            i++;
            this.people.put(row[0], new Person(row[0], row[2], row[1], Integer.parseInt(row[3])));
            countPeople++;
        }
        br.close();
    ...
    public List<String> allocate(String hub, int d) {
        int n = d<7?getDailyAvailable(hub, d%7):num;
        List<String> inter = new ArrayList<>();
        for(String s:intervals.keySet()){
            inter.add(s);
        }
        allocated.putIfAbsent(hub, new HashMap<>());
        for(int i = inter.size()-1; i >= 0; i--)
        {
            String str = inter.get(i);
            for (int count=jx=0; jx < (int) (n*(d<7?0.4:1)) && count<intervals.get(str).size(); count++) {
                String p = intervals.get(str).get(count);
                if(people.get(p).getAll()==0 && (jx++ > -1))
                    if((allocated.get(hub).put(p,d%7))==null) people.get(p).setAll(1);
            }
            n = n - jx;
        }
        if((num = n) != 0) allocate(hub, d+7);
        return allocated.get(hub).entrySet().stream().filter(e->e.getValue()==d%7).map(e->e.getKey()).collect(toList());
    }
    ...
    public void clearAllocation() {
        ...
        for(Person p:people.values())p.setAll(0);
    }
    ...
    public List<Map<String, List<String>>> weekAllocate() {
        List<Map<String, List<String>>> l = new LinkedList<>();
        for(int i = 0; i < 7 && l.add(new HashMap<>()); i++) for (String h : hubs.keySet()) l.get(i).put(h, allocate(h, i));
        return l;
    }
    ...
    public double propAllocated() {
        return (double)people.values().stream().filter(p->p.getAll()==1).count()/countPeople;
    }
    ...
    public Map<String, Double> propAllocatedAge() {
        return intervals.keySet().stream().collect(toMap(s->s,s->(double)intervals.get(s).stream().filter(p->people.get(p).getAll()==1).count()/countPeople));
    }
    ...
    public Map<String, Double> distributionAllocated() {
        return propAllocatedAge().entrySet().stream().collect(toMap(e -> e.getKey(), e -> e.getValue()/propAllocated()));
    }

    BiConsumer<Integer,String> lx = null;
    // R6
    /**
